        ...
        public IActionResult InsertOrUpdate(PersonViewModel model)
        {
            ...
            else
            {
                ...
                if (model.Id <= 0)
                {
                    ...
                    {
                        Errors.Add(new ErrorResult() { Field = "Ex", ErrorMessage = "Ngày cấp CMT sai định dạng dd/MM/yyyy" });
                ...
                {
                    //HUNGVX-REPLACE DATETIME EDIT
                    var accountOld = _accountService.Raw_Get(model.Id);
                    //model.Birthday = model.Birthday.Replace('/', '-');
                    //model.StartDateOffical = model.StartDateOffical.Replace('/', '-');
                    //model.IdCardIssuedDate = model.IdCardIssuedDate.Replace('/', '-');
                    //if(!string.IsNullOrEmpty(model.StartDateOfProbation)) model.StartDateOfProbation = model.StartDateOfProbation.Replace('/', '-');
                    if (model.Birthday != null && !Utils.CheckDateTime(model.Birthday))
                    {
                        Errors.Add(new ErrorResult() { Field = "Ex", ErrorMessage = "Ngày sinh sai định dạng dd/MM/yyyy" });
