...
{
    [Area("Admin")]
    [Route("Admin/SaleRevenueStatistics")]
    public class SaleRevenueStatisticsController : BaseController
    {
        ...
        public SaleRevenueStatisticsController(IHttpContextAccessor httpContextAccessor, ILogger<BaseController> logger, IStatistic iStatis, ITarget iTarget, IBranch iBranch) : base(httpContextAccessor, logger)
        {
            _iStatis = iStatis;
            _iTarget = iTarget;
            _iBranch = iBranch;
        ...
        public IActionResult Index()
        {
            ...
            //ViewBag.MyCode = tokenModel.BranchCode;
        ...
        #region GetStatisticsData-Branches
        [HttpGet]
        [Route("GetStatisticsData/{time}/{timeOption}")]
        public IActionResult GetStatisticsData(string time, int timeOption)
        {
            //trace log
            ...
            var getRevenueInDurationsResult =
                _iStatis.GetRevenueStatisticsInDurations(statisticalDate, timeOption);
            ...
            var getRevenueOfALevelResult =
                _iStatis.GetBranchesRevenueStatistics(statisticalDate, timeOption);
            ...
            var getCurrentTotalRevenueResult =
                _iStatis.GetCurrentRevenueAndProportion(statisticalDate, timeOption);
            ...
            //get revenue target by time option
            var getTarget =
                _iTarget.GetRevenueTarget(7, null, statisticalDate, timeOption);
            if (getTarget.Error)
            {
                //write trace log
            ...
            var getProductComponents =
                _iStatis.GetProductComponents(statisticalDate, timeOption);
            ...
            //get revenue target of branches
            var getBranchTargets =
                _iTarget.GetBranchRevenueTargetList(statisticalDate);
            if (getBranchTargets.Error)
            {
                //write trace log
                LogModel.ItemName = "branch revenue target";
                LogModel.Result = ActionResultValue.ThrowException;
                Logger.LogError(LogModel.ToString());
            ...
            return Json(new
            {
                ...
                Data = new RevenueStatisticViewModel
                {
                    ...
                    Targets = getBranchTargets.Result.ToList()
                }
            });
        }
        #endregion
        ...
        public IActionResult SetRevenueTarget(SaleTargetViewModel model)
        {
            ...
            if (model != null && model.Targets != null)
            {
                foreach (var item in model.Targets)
                {
                    ...
                    try
                    {
                        var getBranchResult = _iBranch.GetByCode(item.SetTargetFor);

                        if (string.IsNullOrEmpty(getBranchResult.DataItem.BranchName))
                        {
                            //write trace log
                            LogModel.Data = new { BranchCode = item.SetTargetFor }.ToDataString();
                            LogModel.ItemName = "branch";
                            LogModel.Action = ActionType.GetInfo;
                            LogModel.Result = ActionResultValue.NotFoundData;
                            Logger.LogWarning(LogModel.ToString());

                            return Json(new { Result = 400, Errors = new List<string> { "Không tìm thấy thông tin chi nhánh" } });
                        }
                    }
                    catch 
                    {
