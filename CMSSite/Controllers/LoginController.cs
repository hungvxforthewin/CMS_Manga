...
namespace CRMSite.Controllers
{
    public class LoginController : Controller
    {
        ...
        public async Task<IActionResult> LoginAccount(LoginAccount model)
        {
            ...
            if (res.Result.Count == 0 || res.Result.First().Pass != passwEncode) 
            {
                //write trace log
            ...
            var claims = new List<Claim>
            {
                ...
                new Claim(SiteConst.TokenKey.FULLNAME, user.FullName ?? string.Empty),
                new Claim(ClaimTypes.Role, user.Role.ToString()),
                new Claim(ClaimTypes.Authentication, user.CodeStaff),
                new Claim(ClaimTypes.NameIdentifier, user.Id.ToString()),
                new Claim(SiteConst.TokenKey.BRANCHCODE, user.BranchCode?.ToString() ?? string.Empty),
                new Claim(SiteConst.TokenKey.OFFICECODE, user.OfficeCode?.ToString() ?? string.Empty),
                new Claim(SiteConst.TokenKey.DEPARTMENTCODE, user.DepartmentCode?.ToString() ?? string.Empty),
                new Claim(SiteConst.TokenKey.TEAMCODE, user.TeamCode?.ToString() ?? string.Empty),
                new Claim(SiteConst.TokenKey.USERNAME, user.UserName),
            };
            ...
            _logModel.Username = user.UserName;
