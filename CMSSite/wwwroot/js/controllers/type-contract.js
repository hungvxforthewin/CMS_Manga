let baseUrl = '/Admin/TypeContract/';
$(function () {
    // search
    $('#search-type-contract').on('click', function () {
    ...
    $('.module-content .search input').keypress(function (e) {
        ...
        {
            $('#search-type-contract').trigger('click');
        }
    });


    // event for button add
    $('#Add-Type-Contract').on('click', function () {
        $('#admin_contract-category_add-modal input').val('');
    });

    $('#admin_contract-category_add-modal #create-type-contract').click(function () {
        $('.loading-wrapper').show();
        ...
        $.post(baseUrl + 'Create', data, function (res) {
            if (res.result == 200) {
                $('#search-type-contract').trigger('click');
                CloseModal('#admin_contract-category_add-modal');
    ...
    $('#Edit-Type-Contract').on('click', function () {
        $('.loading-wrapper').show();
        let selectedRow = $('input[name="select-row"]:checked');
        if (selectedRow.length != 1) {
            toastr.error('Chọn một loại hợp đồng để cập nhật');
            $('.loading-wrapper').hide();
            return;
        }
        ...
        if (id) {
            $.get(baseUrl + 'Update?id=' + id, function (res) {
                ...
                if (res.result == 400) {
                    ...
                    $('#admin_contract-category_edit-modal .content-modal').html(res);
                    ShowModal('#admin_contract-category_edit-modal');
                    $('.loading-wrapper').hide();

                    $('#admin_contract-category_edit-modal #update-type-contract').click(function () {
                        $('.loading-wrapper').show();
                        var data = $('#update-form').serializeArray();
                        $.post(baseUrl + 'Update', data, function (res) {
                            if (res.result == 200) {
                                $('#search-type-contract').trigger('click');
                                CloseModal('#admin_contract-category_edit-modal');
                                toastr.success(res.message);
                            }
                            else {
                                toastr.error(res.errors.join('<br />'));
    ...
    $('#Delete-Type-Contract').on('click', function () {
        $('.loading-wrapper').show();
        let selectedRow = $('input[name="select-row"]:checked');
        if (selectedRow.length != 1) {
            toastr.error('Chọn một loại hợp đồng để xóa');
            $('.loading-wrapper').hide();
            return;
        }
        let id = selectedRow.data('id');
        if (id) {
            $.get(baseUrl + 'Delete?id=' + id, function (res) {
                ...
                if (res.result == 400) {
                    ...
                    $('#admin_contract-category_delete-modal .content-modal').html(res);
                    ShowModal('#admin_contract-category_delete-modal');
                    $('.loading-wrapper').hide();

                    $('#admin_contract-category_delete-modal #delete-type-contract').click(function () {
                        $.get(baseUrl + 'ConfirmDelete?id=' + id, function (res) {
                            if (res.result == 200) {
                                $('#search-type-contract').trigger('click');
                                CloseModal('#admin_contract-category_delete-modal');
                                toastr.success(res.message);
                            }
                            else {
                                toastr.error(res.errors.join('<br />'));
