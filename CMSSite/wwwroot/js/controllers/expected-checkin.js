...
$(function () {
    ...
    $('#Edit-CheckInOut').on('click', function () {
        ...
        if (selectedRow.length != 1) {
            toastr.error('Chọn một khách hàng dự kiến để cập nhật');
    ...
    $('#View-CheckInOut').on('click', function () {
        ...
        let selectedRow = $('#table-body tr.selected');
        if (selectedRow.length != 1) {
            toastr.error('Chọn một khách hàng dự kiến để xem chi tiết');
            $('.loading-wrapper').hide();
            return;
        }
        let id = selectedRow.data('id');
    ...
    $('#checkin-showup').on('click', function () {
        ...
        $('#myModal_checkin_accept input[name="PhoneNumber"]').val('');
    ...
    $('#checkout-showup').on('click', function () {
        ...
        $('#myModal_checkout input[name="PhoneNumber"]').val('');
    ...
    $('#import-data').on('click', function () {
        ...
        if (!file) {
            toastr.warning("Chưa chọn file chứa danh sách khách hàng dự kiến");
            $('.loading-wrapper').hide();
            return;
        }

        toastr.info("Vui lòng chờ hệ thống đang nhập danh sách khách hàng dự kiến");
    ...
    $('#Delete-CheckInOut').on('click', function () {
        ...
        let selectedRow = $('#table-body tr.selected');
        if (selectedRow.length != 1) {
            toastr.error('Chọn một khách hàng dự kiến để xóa');
            $('.loading-wrapper').hide();
            return;
        }
        let id = selectedRow.data('id');
        if (id) {
            $.get(baseUrl + 'Delete?id=' + id, function (res) {
                if (res.result == 400) {
                    ...
                    $('#myModal_delete .content-modal').html(res);
                    //setup show modal
                    ShowModal('#myModal_delete');
                    $('.loading-wrapper').hide();

                    $('#myModal_delete #btn_delete-ex_investor').click(function () {
                        $('.loading-wrapper').show();
                        
                        $.get(baseUrl + 'ConfirmDelete?id=' + id, function (res) {
                            if (res.result == 200) {
                                $('#search-checkin').trigger('click');
                                CloseModal('#myModal_delete');
                                toastr.success(res.message);
                            }
                            else {
                                toastr.error(res.errors.join('<br />'));
    ...
    $('#pagination').pagination({
        ajax: function (options, refresh, $target) {
            if (!data[4]) {
                ...
                data[4].value = options.current;
            }
            if (!data[5]) {
                data.push({ name: "Size", value: $('#size-page select').val() });
            }
            else {
                data[5].value = $('#size-page select').val();
            }
