...
{
    public class ShowUpHistoryImp : BaseService<ShowUpHistory, long>, IShowUpHistory
    {
        #region Const
        ...
        public DataResult<ShowUpHistoryTableViewModel> GetList(SearchShowUpHistoryModel model, out int total)
        {
            ...
            param.Add("@Product", model.Product);
            param.Add("@InvestorResource", model.InvestorResource);
            param.Add("@TeleSale", model.TeleSale);
            param.Add("@CheckIn", model.CheckInDate);
            param.Add("@CheckOut", model.CheckOutDate);
            param.Add("@Branch", model.Branch);
            param.Add("@CC", model.Sale);
            param.Add("@CLTO", model.SaleTO);
            param.Add("@Total", dbType: DbType.Int32, direction: ParameterDirection.Output);
            total = 0;
            try
            {
                var lst = this.Procedure<ShowUpHistoryTableViewModel>("sp_tblShowUpHistory_GetList", param).ToList();
                    ...
                    using (var transaction = db.BeginTransaction())
                    {
                        foreach (var item in model)
                        {
                            ...
                            using (var multipleresult = db.QueryMultiple("sp_tblShowUpHistory_GetInfo_By_PhoneNumber_Event", 
                                new { @phone = investor.PhoneNumber, @Event = item.CodeEvent }, transaction, 
                                commandType: CommandType.StoredProcedure))
                            {
